          ...
          Geom<T> geom)
{
  ...
  // First and last pixel of each ROI (rounded for compatibility with the Caffe definition).
  int roi_image = (int)rois[0];

  // tmp fix to numerically match caffe
  T u1 = static_cast<T>(round(u1_-1)) * geom.transform[0] ;
  T v1 = static_cast<T>(round(v1_-1)) * geom.transform[3] ;
  T u2 = static_cast<T>(round(u2_)) * geom.transform[0] ; // match +1 for width
  T v2 = static_cast<T>(round(v2_)) * geom.transform[3] ; // match +1 for width
  ...
  /*int c = (pc * geom.subdivisions[1] + pw) * geom.subdivisions[0] + ph ;*/
  ...
  if (outputIndex == 1) {
      printf("pre-scaled rois:\n") ;
      printf("pre-scaled rois: %g \n", u1_) ;
      printf("pre-scaled rois: %g \n", v1_) ;
      printf("pre-scaled rois: %g \n", u2_) ;
      printf("pre-scaled rois: %g \n", v2_) ;

      printf("post-scaled rois:\n") ;
      printf("u1: %f \n", u1) ;
      printf("v1: %f \n", v1) ;
      printf("u2: %f \n", u2) ;
      printf("v2: %f \n", v2) ;
      ...
      printf("roi window:\n") ;
      printf("roi_start_h: %f \n", roi_start_h) ;
      printf("roi_start_w: %f \n", roi_start_w) ;
      printf("roi_height: %f \n", roi_height) ;
      printf("roi_width: %f \n", roi_width) ;
