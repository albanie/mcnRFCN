...
struct acc_max
{
  ...
  /// DEBUG -> REMOVE
  inline type check_scale() const { return value ; }
  inline type check_value() const { return value ; }
  /// DEBUG -> REMOVE
...
struct acc_sum
{
  ...
  /// DEBUG -> REMOVE
  inline type check_scale() const { return scale ; }
  inline type check_value() const { return value ; }
  /// DEBUG -> REMOVE
        ...
        for (int z = 0; z < outChannels; ++z) {
          ...
          for (int pw = 0; pw < subdivisions[1]; ++pw) {
            ...
            for (int ph = 0; ph < subdivisions[0]; ++ph) {
             ...
             if (roi == 0 && z==1 && pw==0 && ph==0) {
                printf("CPU:: z: (%d)\n", z) ;
                printf("CPU:: roi_start_w: (%f)\n", roi_start_w) ;
                printf("CPU:: roi_start_h: (%f)\n", roi_start_h) ;
                printf("CPU:: roi_end_w: (%f)\n", roi_end_w) ;
                printf("CPU:: roi_end_h: (%f)\n", roi_end_h) ;
                printf("CPU:: (hstart,hend,wstart,wend): (%d,%d,%d,%d)\n", 
                              hstart,hend,wstart,wend) ;
                printf("CPU:: offset: (%d)\n", off) ;
                for (int w = wstart ; w < wend; ++w) {
                  for (int h = hstart ; h < hend; ++h) {
                    const int index_ = off + w * height + h ;
                    printf("CPU:: (h,w): (%d,%d): %g\n",
                               h, w, data_offset[index_]) ;
                  }
                }
             }
              if (is_empty) {
                *pooled++ = 0 ;
                ...
                // NOTE: The definition of the boundaries has 
                // changed slightly, so the accumulator must be adjusted
                ...
                type res = acc.done_forward() ;
                *pooled++ = res ;
                //if (std::abs(res) > 1000) {
                 //if (hstart==32&&hend==34&&wstart==38&&wend==40) {
                 //if (hstart==32&&hend==34) {
                  //printf("CPU:: (hstart,hend,wstart,wend): (%d,%d,%d,%d)\n", 
                              //hstart,hend,wstart,wend) ;
                  //printf("CPU:: (z,w,h): (%d,%d,%d) val: %f\n", z,pw,ph,res) ;
                  //for (int w = wstart ; w < wend; ++w) {
                    //for (int h = hstart ; h < hend; ++h) {
                      //const int index_ = off + w * height + h ;
                      //printf("CPU:: (h,w): (%d,%d): %g\n",
