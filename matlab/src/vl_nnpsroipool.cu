// @file vl_nnpsroipooling.cu
// @brief position sensitive roipooling block implementation (GPU)
...
VLMXOption  options [] = {
  ...
  {"OutChannels",      1,   opt_out_channels },
                 ...
                 int nin, mxArray const *in[])
{
  ...
  vl::PSROIPoolingMethod method = vl::vlPSROIPoolingAverage ;
  bool backMode = false ;
  int verbosity = 0 ;
  int opt ;
  int next = IN_END ;
    ...
    switch (opt) {
      ...
      case opt_method : {
        ...
        if (vlmxIsEqualToStringI(optarg, "max")) {
          method = vl::vlPSROIPoolingMax ;
        } else if (vlmxIsEqualToStringI(optarg, "avg")) {
          method = vl::vlPSROIPoolingAverage ;
        } else {
      ...
      case opt_out_channels : {
        if (!vlmxIsPlainScalar(optarg)) {
          vlmxError(VLMXE_IllegalArgument, 
                  "OUTCHANNELS should be a scalar value.") ;
        }
        float n = mxGetScalar(optarg) ; // check for positive integer
        if (!(n == int(n) && int(n) > 0)) {
          vlmxError(VLMXE_IllegalArgument, 
                  "OUTCHANNELS should be a positive integer.") ;
        }
        outChannels = int(n) ;
      }
  ...
  if (verbosity > 0) {
    mexPrintf("vl_nnpsroipool: %s; %s", backMode?"backward":"forward", (data.getDeviceType()==vl::VLDT_GPU) ? "GPU" : "CPU") ;
    mexPrintf("\nvl_nnpsroipool: method: %d; num ROIs: %d\n", method, numROIs);
    mexPrintf("vl_nnpsroipool: subdivisions: [%d x %d]\n", subdivisions[0], subdivisions[1]) ;
    mexPrintf("vl_nnpsroipool: outChannels: %d\n", outChannels) ;
    mexPrintf("vl_nnpsroipool: transform: [%g %g %g ; %g %g %g]\n",
              transform[0], transform[2], transform[4],
              transform[1], transform[3], transform[5]) ;

    vl::print("vl_nnpsroipool: data: ", data) ;
    if (backMode) {
      vl::print("vl_nnpsroipool: derOutput: ", derOutput) ;
      vl::print("vl_nnpsroipool: derData: ", derData) ;
    } else {
      vl::print("vl_nnpsroipool: output: ", output) ;
      vl::print("vl_nnpsroipool: rois: ", rois) ;
    }
  }
  ...
  vl::ErrorCode error ;
  if (!backMode) {
    error = vl::nnpsroipooling_forward(context,
                                       output, 
                                       data, 
                                       rois,
                                       method,
